...
const { EthereumTxValidator } = require('../lib/TxValidator');
...
describe("EthereumBlockchain.isTransactionValid", () => {
  ...
  test('for valid blockchainTransaction', async () => {
    ...
    const validResults = await txValidator.isTransactionValid(blockchainTransaction, hwAddress)
  ...
  test('for incorrect txRaw', async () => {
    ...
    const validResults = await txValidator.isTransactionValid(bt, hwAddress)
