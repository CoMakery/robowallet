...
class AlgorandTxValidator {
  ...
  async isTransactionValid(blockchainTransaction, hotWalletAddress) {
    ...
    const { txRaw, blockchainTransactables } = blockchainTransaction
    try {
      const transaction = JSON.parse(blockchainTransaction.txRaw)
      const transactableType = blockchainTransactables[0].blockchainTransactableType
  ...
  async isCoinsBalanceEnough({ txCoinsAmount, hotWalletAddress }) {
    ...
    const mAlgosBalance = hwCoinsBalance.multipliedBy(1000000)
    const txAmount = new BigNumber(txCoinsAmount)

    if (txAmount.isLessThanOrEqualTo(mAlgosBalance)) {
      return true
    } else {
      const errorMessage = `The Hot Wallet has insufficient coins to transfer (${mAlgosBalance} < ${txAmount})`
      console.log(errorMessage)
      this.markAs = "cancelled"
      this.error = errorMessage
      return false
