...
exports.checkAllVariablesAreSet = function checkAllVariablesAreSet(envs) {
  ...
  } else if (["algorand", "algorand_test", "algorand_beta"].indexOf(envs.blockchainNetwork) > -1) { // Algorand
    return Boolean(envs.purestakeApi) && Boolean(envs.optInApp)
  } else if (["solana", "solana_devnet", "solana_testnet"].indexOf(envs.blockchainNetwork) > -1) { // Solana
    return Boolean(envs.figmentApiKey)
  } else {
    return false
  }
}
...
exports.waitForNewTransaction = async function waitForNewTransaction(envs, hwRedis) {
  ...
  if (blockchainTransaction.disableWalletWith) {
    const withdrawalAddresses = blockchainTransaction.disableWalletWith
    const disableResult = await blockchain.disableWallet(withdrawalAddresses, hotWallet)
  ...
  if (Object.keys(blockchainTransaction).length === 0) return {}
