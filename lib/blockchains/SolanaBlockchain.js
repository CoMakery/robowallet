...
class SolanaBlockchain {
  ...
  withdrawTokensTx(hotWalletAddress, mintAddress, withdrawalAddress, tokensAmount) {
    return {
  ...
  async sendTransaction(blockchainTransaction, hotWallet) {
    ...
    try {
      ...
      console.error(err)
      console.error(err.stack)
      if (err.name === 'SolanaTransactionSizeError') {
  ...
  async disableWallet(disableValues, hotWallet) {
    ...
    const mintAddress = disableValues.mintAddress || disableValues.tokenContract
    if (disableValues.tokenAddress && mintAddress) {
      const tokenBalance = await this.getTokenBalance(hotWallet.address, mintAddress)

      if (tokenBalance.balanceInBaseUnit.isGreaterThan(new BigNumber(0))) {
        const tokensAmount = tokenBalance.balanceInBaseUnit
        const txObject = this.withdrawTokensTx(hotWallet.address, mintAddress, disableValues.tokenAddress, tokensAmount)
