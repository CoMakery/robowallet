  ...
  async disableWallet(withdrawalAddresses, hotWallet) {
    ...
    if (withdrawalAddresses.coinAddress && result.tokensWithdrawalTx.status !== "failed") {
      ...
      if (coinBalance.isGreaterThan(new BigNumber(0.02))) {
        ...
        const maxPriorityFeePerGas = web3.utils.toHex(web3.utils.toWei(this.envs.ethereumMaxPriorityFeePerGas, 'gwei' )) * 21000
        const maxFeePerGas = web3.utils.toWei(this.envs.ethereumMaxFeePerGas, 'gwei' ) * 21000
        const reserveAmount = new BigNumber(0.001)
        const coinsAmount = web3.utils.toWei(coinBalance.minus(reserveAmount).toString(), 'ether') - baseFee - maxFeePerGas - maxPriorityFeePerGas
