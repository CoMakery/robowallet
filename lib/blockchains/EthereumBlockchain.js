  ...
  async disableWallet(withdrawalAddresses, hotWallet) {
    ...
    if (withdrawalAddresses.coinAddress && result.tokensWithdrawalTx.status !== "failed") {
      ...
      if (coinBalance.isGreaterThan(new BigNumber(0.02))) {
        ...
        const maxFeePerGas = web3.utils.toWei(this.envs.ethereumMaxFeePerGas, 'gwei' )
        const reserveAmount = new BigNumber(0.01)
        const coinsAmount = web3.utils.toWei(coinBalance.minus(reserveAmount).toString(), 'ether') - txFee - baseFee - maxFeePerGas
