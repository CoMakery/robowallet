...
class EthereumBlockchain {
  ...
  async sendWithdrawalCoinsTransaction(coinAddress, totalAmount, hotWallet) {
    const chainTransaction = await this.chain.new.Transaction()
    var amountToTransfer = totalAmount
    ...
    try {
      ...
      const ethFee = await chainTransaction.getSuggestedFee(chainjs.Models.TxExecutionPriority.Fast)
      amountToTransfer = amountToTransfer.minus(new BigNumber(ethFee)).minus(new BigNumber(0.000000000000010))
      ...
      chainTransaction.actions[0] = [await this.chain.composeAction(chainjs.Models.ChainActionType.ValueTransfer, txValues)]

      console.log(`totalAmount: ${totalAmount.toString()}`);
      console.log(`ethFee: ${ethFee}`);
      console.log(`amountToTransfer: ${txValues.amount}`);
  ...
  async disableWallet(withdrawalAddresses, hotWallet) {
    ...
    if (withdrawalAddresses.tokenAddress) {
      ...
      if (tokenBalance.balanceInBaseUnit.isGreaterThan(new BigNumber(0))) {
        const contractAddress = this.envs.ethereumContractAddress.toString()
        ...
        if (typeof tx.valid == 'undefined' || tx.valid) {
          ...
          const msg = `Error during sending tx`
          console.error(msg)
          result.tokensWithdrawalTx = { status: "failed", txHash: null, message: msg }
        }
      } else {
        result.tokensWithdrawalTx = { status: "skipped", txHash: null, message: 'Tokens balance is zero' }
      }
    }

    if (withdrawalAddresses.coinAddress && await this.enoughCoinBalanceToSendTransaction(hotWallet.address)) {
      ...
      if (coinBalance.isGreaterThan(new BigNumber(0.001))) {
        ...
        result.coinsWithdrawalTx = { status: "skipped", txHash: null, message: 'ETH balance is zero' }
      }
    }
