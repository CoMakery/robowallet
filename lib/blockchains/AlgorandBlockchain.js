...
class AlgorandBlockchain {
  ...
  async connect() {
    if (!this.algodClient.isConnected) {
      await this.algodClient.connect()
    }
  }
  ...
  async enoughCoinBalanceToSendTransaction(hotWalletAddress) {
    ...
    return algoBalance.isGreaterThan(new BigNumber(1000)) // 1000 microalgos
  }
  ...
  async sendTransaction(transaction, hotWallet) {
    ...
    try {
      ...
      console.log(`Sending transaction: ${JSON.stringify(tx)}`);
  ...
  async disableWallet(withdrawalAddresses, hotWallet) {
    ...
    if (withdrawalAddresses.tokenAddress) {
      ...
      // todo: validate behaviour if clear app twice  
      if (this.envs.optInApp) {
      ...
      // todo: try to close out app for account after successful clear
    }

    if (withdrawalAddresses.coinAddress) {
      const txParams = await this.algodClient.getTransactionParams().do()
      const withdrawalFee = 2 * txParams.fee
      const coinBalance = await this.getAlgoBalanceForHotWallet(hotWallet.address)
      if (coinBalance.isGreaterThan(new BigNumber(withdrawalFee)) && result.tokensWithdrawalTx.status !== "failed") {
        ...
        const amountWithoutFee = coinBalance - withdrawalFee //- 1000000
        // wait 1 sec to not get too many requests error
        await new Promise(resolve => setTimeout(resolve, 1000))
