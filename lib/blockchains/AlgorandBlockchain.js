...
class AlgorandBlockchain {
  ...
  async isTransactionValid(blockchainTransaction, hotWalletAddress) {
    ...
    try {
      console.log(blockchainTransaction.txRaw)
      ...
      if (transaction.appArgs && transaction.appArgs[0] === "0x7472616e73666572") {
        ...
        if (this.envs.maxAmountForTransfer > 0 && txTokensAmount > this.envs.maxAmountForTransfer) {
          const errorMessage = `The transaction has too big amount to transfer (${txTokensAmount}). Max amount is ${this.envs.maxAmountForTransfer}`
          console.log(errorMessage)
          return { valid: false, markAs: "failed", error: errorMessage }
        }
